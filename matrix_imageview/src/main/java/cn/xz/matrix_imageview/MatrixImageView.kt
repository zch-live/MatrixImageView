package cn.xz.matrix_imageview
...
import cn.xz.matrix_imageview.utils.MatrixImageUtils
import cn.xz.matrix_imageview.utils.MatrixImageUtils.callRotation
import cn.xz.matrix_imageview.utils.MatrixImageUtils.getDistanceOf2Points
import cn.xz.matrix_imageview.utils.MatrixImageUtils.getImageRectF
import cn.xz.matrix_imageview.utils.MatrixImageUtils.getTouchMode
import cn.xz.matrix_imageview.utils.coroutineDelay
...
class MatrixImageView : AppCompatImageView {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event.action.and(ACTION_MASK)) {
            ACTION_DOWN -> {
                ...
                if (touchMode == MatrixImageUtils.TouchMode.TOUCH_OUTSIDE) {
                    mShowFrame = false
                    invalidate()
                    return super.onTouchEvent(event)
                }
                ...
                if (touchMode == MatrixImageUtils.TouchMode.TOUCH_ROTATE) {
                    // 旋转角度
    ...
    private fun touchMove(x: Float, y: Float, imageRect: RectF) {
        ...
        when (touchMode) {
            MatrixImageUtils.TouchMode.TOUCH_IMAGE -> {
                mImgMatrix.postTranslate(x - mLastX, y - mLastY)
                imageMatrix = mImgMatrix
                return
            }
            MatrixImageUtils.TouchMode.TOUCH_ROTATE -> {
                // 图片中心点x坐标
                val centerX = (imageRect.right - imageRect.left) / 2 + imageRect.left
                // 图片中心点y坐标
                val centerY = (imageRect.bottom - imageRect.top) / 2 + imageRect.top
            ...
            MatrixImageUtils.TouchMode.TOUCH_CONTROL_1 -> {
                // 缩小
            ...
            MatrixImageUtils.TouchMode.TOUCH_CONTROL_2 -> {
                // 缩小
                scaleFactorX = if (x - mLastX < 0) {
                    (totalTransOblique - scaleTrans) / totalTransOblique
                } else {
            ...
            MatrixImageUtils.TouchMode.TOUCH_CONTROL_3 -> {
                // 缩小
                scaleFactorX = if (x - mLastX > 0) {
                    (totalTransOblique - scaleTrans) / totalTransOblique
                } else {
            ...
            MatrixImageUtils.TouchMode.TOUCH_CONTROL_4 -> {
                // 缩小
                scaleFactorX = if (x - mLastX < 0) {
                    (totalTransOblique - scaleTrans) / totalTransOblique
                } else {
            ...
            MatrixImageUtils.TouchMode.TOUCH_CONTROL_5 -> {
                // 缩小
                scaleFactorX = if (x - mLastX > 0) {
            ...
            MatrixImageUtils.TouchMode.TOUCH_CONTROL_6 -> {
                // 缩小
                scaleFactorX = if (x - mLastX < 0) {
                    (totalTransHorizontal - scaleTrans) / totalTransHorizontal
                } else {
            ...
            MatrixImageUtils.TouchMode.TOUCH_CONTROL_7 -> {
                // 缩小
                scaleFactorX = 1f
